...
(check `.opencode/skills/<resolved-name>/SKILL.md` or `skills/<resolved-name>/SKILL.md`).
...
- If evolving a **specific skill** (e.g., `/aif-evolve plan`) → read only that one:
  `Read: .opencode/skills/aif-plan/SKILL.md` (or `skills/aif-plan/SKILL.md` if not installed)
- If evolving **all skills** (`/aif-evolve` or `/aif-evolve all`) → read all:
  `Glob: .opencode/skills/*/SKILL.md` (or `Glob: skills/*/SKILL.md` if not installed)
